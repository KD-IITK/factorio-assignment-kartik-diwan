...
TEMP_INPUT = os.path.join(script_dir, "temp_belt_input.json")
TEMP_EXPECTED = os.path.join(script_dir, "temp_belt_expected.json")
TEMP_ACTUAL = os.path.join(script_dir, "temp_belt_actual.json")

def main():
    print("Starting belts test runner (Python)...")
    
    test_cases_file = os.path.join(script_dir, "belts_test_cases.json")
    script_to_test = os.path.join(script_dir, "..", "belts", "main.py")
    compare_script = os.path.join(script_dir, "compare_json.py")
    
    # Check that all required files exist
    required_files = [test_cases_file, script_to_test, compare_script]
    for f in required_files:
        if not os.path.exists(f):
            print(f"{COLOR_RED}FATAL: Required file '{f}' not found.{COLOR_RESET}")
            print(f"Please make sure '{f}' is in the correct location.")
            sys.exit(1)
